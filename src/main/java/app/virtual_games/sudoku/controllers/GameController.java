...
public class GameController extends Application
{
	...
	private static ObservableList<String> puzzleDifficulties;
	...
	{
		puzzleDifficulties = PuzzleDifficulty.getPuzzleDifficulties();	// initialize difficulty variables
...
//			LOGGER.log(Level.SEVERE, e.getMessage());
	...
	public static void startNewSudoku()
	{
		try 
		{
			...
			else 
			{
				LOGGER.log(Level.SEVERE, "No sudoku solution found");
				stopGameTimer();
				loadErrorDialog(false);									// false —> Game Screen
			}
		}
		catch (Exception e) 
		{
	...
	public static void restartSudoku()
	{
		...
		if (isWritingToolClicked()) { unclickWritingTool(); }
	...
	public static void enableGameScreenElements()
	{
		...
		numberButtons.forEach(button -> enableNumberButton(button));
	 ...
	 * 
	 */
	private static void initializeGameScreenVariables() throws Exception
	{
    ...
    {
    	Timeline timer = new Timeline(new KeyFrame(Duration.seconds(1), new TimerHandler()));
    ...
    {
    	Timeline timer = new Timeline(new KeyFrame(Duration.seconds(3), new TimePenaltyHandler()));
    ...
    {
    	Timeline timer = new Timeline(new KeyFrame(Duration.seconds(3), new HintCellHandler()));
	 ...
	 * 
	 */
	private static void loadGameScreen() throws Exception
	{
	...
	private static void loadGameScreenElements() 
	{
		...
		writingToolButtons = ((GameScreen) currentScreen).getWritingToolButtons();
		helperButtons = ((GameScreen) currentScreen).getHelperButtons();
		numberButtons = ((GameScreen) currentScreen).getNumberButtons();
		disabledNumberButtons = ((GameScreen) currentScreen).getDisabledNumberButtons();
	}
	 ...
	 * 
	 */
	public static ArrayList<NumberButton> getNumberButtons() 
	{
	 ...
	 * 
	 */
	public static ArrayList<NumberButton> getDisabledNumberButtons() 
	{
		return disabledNumberButtons;
	}
}